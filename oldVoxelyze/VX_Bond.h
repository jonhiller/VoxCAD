...
public:
	...
	template <typename T> void ToXDirBond(Quat3D<T>* const pQuat) const {switch (ThisBondAxis){case AXIS_Y: {T tmp = pQuat->x; pQuat->x=pQuat->y; pQuat->y = -tmp; break;} case AXIS_Z: {T tmp = pQuat->x; pQuat->x=pQuat->z; pQuat->z = -tmp; break;}}}
	...
	template <typename T> void ToOrigDirBond(Quat3D<T>* const pQuat) const {switch (ThisBondAxis){case AXIS_Y: {vfloat tmp = pQuat->y; pQuat->y=pQuat->x; pQuat->x = -tmp; break;} case AXIS_Z: {vfloat tmp = pQuat->z; pQuat->z=pQuat->x; pQuat->x = -tmp; break;}}};
	template <typename T> Vec3D<T> toAxisX		(const Vec3D<T>& v)	const {switch (ThisBondAxis){case AXIS_Y: return Vec3D<T>(v.y, -v.x, v.z); case AXIS_Z: return Vec3D<T>(v.z, v.y, -v.x); default: return v;}} //transforms a vec3D in the original orientation of the bond to that as if the bond was in +X direction
	template <typename T> Quat3D<T> toAxisX		(const Quat3D<T>& q)	const {switch (ThisBondAxis){case AXIS_Y: return Quat3D<T>(q.w, q.y, -q.x, q.z); case AXIS_Z: return Quat3D<T>(q.w, q.z, q.y, -q.x); default: return q;}} //transforms a vec3D in the original orientation of the bond to that as if the bond was in +X direction
