...
#include "../Voxelyze/include/VX_Link.h"
...
//	int x, y, z;
	...
	for (int i = 0; i<NumVox; i++) //go through all the voxels...
	{
		...
		{
			switch (i){
			case BD_PX: PointToDrawFrom = Center + Angle.RotateVec3D(Vec3D<>(0.1*vSize, 0, 0)); break;
			case BD_NX: PointToDrawFrom = Center + Angle.RotateVec3D(Vec3D<>(-0.1*vSize, 0, 0)); break;
			case BD_PY: PointToDrawFrom = Center + Angle.RotateVec3D(Vec3D<>(0, 0.1*vSize, 0)); break;
			case BD_NY: PointToDrawFrom = Center + Angle.RotateVec3D(Vec3D<>(0, -0.1*vSize, 0)); break;
			case BD_PZ: PointToDrawFrom = Center + Angle.RotateVec3D(Vec3D<>(0, 0, 0.1*vSize)); break;
			case BD_NZ: PointToDrawFrom = Center + Angle.RotateVec3D(Vec3D<>(0, 0, -0.1*vSize)); break;
			};
...
void CVXS_SimGLView::DrawGeometry(int Selected, bool ViewSection, int SectionLayer, vfloat ScaleVox)
{
	...
	int indexCounter = 0;
//	for (std::vector<CVX_Voxel*>::const_iterator it = voxels->begin(); it!=voxels->end(); it++){
...
//		CVX_Voxel* pVox = *it;
		...
		CGL_Utils::DrawCube((Vec3D<>)pVox->cornerOffset(CVX_Voxel::NNN)*ScaleVox, (Vec3D<>)pVox->cornerOffset(CVX_Voxel::PPP)*ScaleVox, true, true, 1.0, ThisColor);
//		CGL_Utils::DrawCube(pVox->cornerNegative()*ScaleVox, pVox->cornerPositive()*ScaleVox, true, true, 1.0, CColor(pVox->material()->red()/255.0, pVox->material()->green()/255.0, pVox->material()->blue()/255.0, 1.0));
		glPopMatrix();
	}
#else
	Vec3D<> Center;
...
{
	if (Selected) return CColor(1.0f, 0.0f, 1.0f, 1.0f); //highlight selected voxel (takes precedence...)
	...
	switch (CurViewCol) {
		...
		case RVC_STATE:
			if (pVox->isFailed()) return CColor(1.0f, 0.0f, 0.0f, 1.0f);
			else if (pVox->isYielded()) return CColor(1.0f, 1.0f, 0.0f, 1.0f);
		...
		case RVC_STRAIN_EN: {
			...
			for (int i=0; i<6; i++){
				...
				if (pL && pL->strainEnergy() > maxSE) maxSE = pL->strainEnergy();
			}
		...
		case RVC_STRAIN: {
			...
			for (int i=0; i<6; i++){
				CVX_Link* pL = pVox->link((CVX_Voxel::linkDirection)i);
				if (pL && pL->axialStrain() > maxSE) maxSE = pL->axialStrain();
			}
		...
		case RVC_STRESS: {
			...
			for (int i=0; i<6; i++){
				CVX_Link* pL = pVox->link((CVX_Voxel::linkDirection)i);
				if (pL && pL->axialStress() > maxSE) maxSE = pL->axialStress();
			}
...
CColor CVXS_SimGLView::GetInternalBondColor(CVX_Link* pLink)
{
	switch (CurViewCol) {
		case RVC_TYPE:
			if (pLink->isSmallAngle()) return CColor(0.3, 0.7, 0.3, 1.0);
		...
		case RVC_STATE:
			if (pLink->isFailed()) return CColor(1.0f, 0.0f, 0.0f, 1.0f);
			else if (pLink->isYielded()) return CColor(1.0f, 1.0f, 0.0f, 1.0f);
			else return CColor(0.8f, 0.8f, 0.8f, 1.0f);
			break;
		case RVC_STRAIN_EN:
			if (pSim->SS.MaxBondStrainE == 0) return GetJet(0);
...
{
	switch (CurViewCol) {
		case RVC_TYPE: defMeshVx2->updateMesh(CVX_MeshRender::MATERIAL); break;
		case RVC_KINETIC_EN: defMeshVx2->updateMesh(CVX_MeshRender::STATE_INFO, CVoxelyze::KINETIC_ENERGY); break;
		case RVC_DISP: defMeshVx2->updateMesh(CVX_MeshRender::STATE_INFO, CVoxelyze::DISPLACEMENT); break;
		case RVC_STATE: defMeshVx2->updateMesh(CVX_MeshRender::FAILURE); break;
		case RVC_STRAIN_EN: defMeshVx2->updateMesh(CVX_MeshRender::STATE_INFO, CVoxelyze::STRAIN_ENERGY); break;
		case RVC_STRAIN: defMeshVx2->updateMesh(CVX_MeshRender::STATE_INFO, CVoxelyze::ENG_STRAIN); break;
		case RVC_STRESS: defMeshVx2->updateMesh(CVX_MeshRender::STATE_INFO, CVoxelyze::ENG_STRESS); break;
		case RVC_PRESSURE: defMeshVx2->updateMesh(CVX_MeshRender::STATE_INFO, CVoxelyze::PRESSURE); break;
		default: defMeshVx2->updateMesh(); break;
	}
...
//		if (pLink->isSmallAngle()) ThisColor	= CColor(0.3,0.7,0.3,1.0); //GetInternalBondColor(pBond);
//		else ThisColor	= CColor(0,0,0,1.0); //GetInternalBondColor(pBond);
	...
	//{
		...
		//for (std::list<CVX_Voxel*>::const_iterator jt =colVoxels->begin(); jt!=colVoxels->end(); jt++){
		
		//	CVX_Voxel* pV2 = (*jt);

			CColor ThisColor(0,0,255); // = GetCollisionBondColor(&pSim->BondArrayCollision[i]);
...
void CVXS_SimGLView::DrawAngles(void)
{
	...
	//	Vec3D<> RotAxis1 = (pSim->VoxArray[i].GetCurAngle()*Quat3D<>(Axis1)*pSim->VoxArray[i].GetCurAngle().Conjugate()).ToVec();
...
//		RotAxis1 = (pSim->VoxArray[i].GetCurAngle()*Quat3D<>(Axis1)*pSim->VoxArray[i].GetCurAngle().Conjugate()).ToVec();
		glVertex3d(pSim->VoxArray[i].GetCurPos().x + RotAxis1.x, pSim->VoxArray[i].GetCurPos().y + RotAxis1.y, pSim->VoxArray[i].GetCurPos().z + RotAxis1.z);
