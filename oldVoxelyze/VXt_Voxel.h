...
{
public:
	CVXt_Voxel(CVX_Sim* pSimIn, int SIndexIn, int XIndexIn, int MatIndexIn, Vec3D<>& NominalPositionIn, vfloat OriginalScaleIn);
	~CVXt_Voxel(void);
	CVXt_Voxel(const CVXt_Voxel& VIn) {*this = VIn;} //copy constructor
	CVXt_Voxel& operator=(const CVXt_Voxel& VIn);
...
protected:
	...
	Vec3D<> toGlobal(BondDir dirIn, const Vec3D<>& toTransform);
