...
public:
	...
	const inline float GetCurScale() const {return Scale;}
...
private:
	...
	//Vec3D<double> Scale; //nominal scale based on temperature, poissons deformation, etc.
	...
	//new
	Vec3D<> strain(bool tensionStrain); //LCS returns voxel strain. if tensionStrain true and no actual tension in that
	void updatePoissonsstrain();
