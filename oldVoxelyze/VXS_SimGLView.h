...
//move these to private eventually
	...
	CColor GetJet(vfloat val){
		if (val > 1.0) return CColor(1.0, 0.0, 0.0, 1.0);
		else if (val > 0.75) return CColor(1, 4-val*4, 0, 1.0);
		else if (val > 0.5) return CColor(val*4-2, 1, 0, 1.0);
		else if (val > 0.25) return CColor(0, 1, 2-val*4, 1.0);
